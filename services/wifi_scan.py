...
def parse_wifi_output(raw_output: str) -> list[NetworkRecord]:
    ...
    Args:
        raw_output: The stdout content returned from the netsh Wi-Fi scan.

    Returns:
        A list of parsed network records with SSID, BSSID, signal strength,
        and channel.
    """
    ...
    for line in raw_output.split("\n"):
        ...
        if ssid_match:
            ...
            networks.append(
                NetworkRecord(ssid=current_ssid, mac_address=current_bssid),
            )
...
def scan_wifi_networks() -> list[NetworkRecord]:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            result.stderr.strip() or "netsh returned a non-zero exit code.",
        )
