...
def _validate_command(command: Sequence[str] | str) -> list[str]:
    ...
    Raises:
        ValueError: If the command is empty, blank, or passed as a shell
            string.
        TypeError: If the command is not a sequence of strings.
    """
    if isinstance(command, (str, bytes, bytearray)):
        raise ValueError(
            "Unsafe command: pass a list of arguments instead "
            "of a shell string.",
        )
    ...
    if not normalized_command or any(
        not part.strip() for part in normalized_command
    ):
        raise ValueError(
            "Command cannot be empty or contain blank arguments.",
        )
...
def run_bounded_command(
    ...
    Args:
        ...
        timeout_seconds: Maximum time in seconds allowed for the subprocess.
        max_output_chars: Maximum number of output characters captured from
            stdout and stderr.
